...
#define PROCFS_NAME "GPIO Button interrupt handling with workqueues" 
#define SIZE 1024
...
static void WorkQueueFunction(struct work_struct *work){
    
    pr_info("Starting a work\n");
    msleep(WORKQUEUE_DELAY);
    pr_info("Switching a led\n");
...
static int __init helloInit(void)
{
    ...
    INIT_WORK(&buttonWork, WorkQueueFunction);
